...
function SignupPage() {
    ...
    const handleSubmit = async (event) => {
        ...
        const phoneNumber = event.target.phoneNumber.value; // Added
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_API_BASE}/signup`, {
                ...
                body: JSON.stringify({ email, password, phoneNumber }), // Added phoneNumber
            });
        ...
        <div className="flex items-center justify-center min-h-screen bg-gray-100">
            <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="mb-4">
                        <label htmlFor="phoneNumber" className="block text-sm font-medium text-gray-700 mb-2">Phone Number (Optional)</label>
                        <input
                            type="tel"
                            id="phoneNumber"
                            name="phoneNumber"
                            className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-indigo-500"
                            placeholder="e.g., +1234567890 (for SMS)"
                        />
                    </div>
                    <div className="mb-4">
